...
var POLLUTANT_NAMES = {
  "w01001": "pH", "w01002": "色度", "w01003": "浊度",
  "w01006": "溶解氧", "w01009": "高锰酸盐指数", "w01010": "水温",
  "w01012": "悬浮物", "w01014": "电导率",
  "w01017": "化学需氧量(CODMn)", "w01018": "化学需氧量(CODCr)",
  "w01019": "五日生化需氧量", "w01020": "氨氮",
  "w21001": "总氮", "w21003": "氨氮(废水)", "w21011": "总磷",
  "w21016": "总铬", "w21017": "六价铬",
  "w21019": "总铅", "w21020": "总镉", "w21022": "总汞", "w21023": "总砷",
  "w21038": "石油类", "w99001": "污水流量",
  "a01001": "温度", "a01002": "湿度", "a01006": "气压",
  "a01007": "风速", "a01008": "风向",
  "a01011": "烟气流速", "a01012": "烟气温度", "a01013": "烟气压力", "a01014": "烟气湿度",
  "a05024": "O₃", "a19001": "烟尘",
  "a21002": "NOx", "a21003": "NO", "a21004": "NO₂", "a21005": "CO", "a21026": "SO₂",
  "a21028": "H₂S",
  "a34002": "PM10", "a34004": "PM2.5", "a34013": "烟气黑度",
  "a99001": "废气流量"
};
...
function determineUnit(pollCode) {
  if (pollCode === "w01001") return "无量纲";
  if (pollCode === "w01010" || pollCode === "a01001" || pollCode === "a01012") return "°C";
  if (pollCode === "a01002" || pollCode === "a01014") return "%";
  if (pollCode === "a01006" || pollCode === "a01013") return "Pa";
  if (pollCode === "a01007" || pollCode === "a01011") return "m/s";
  if (pollCode === "a01008") return "°";
  if (pollCode === "w01014") return "μS/cm";
  if (pollCode === "w01003") return "NTU";
  if (pollCode === "w99001" || pollCode === "a99001") return "m³/h";
  if (pollCode.indexOf("a34") === 0 || pollCode.indexOf("a21") === 0 ||
      pollCode.indexOf("a05") === 0 || pollCode.indexOf("a19") === 0) return "mg/m³";
  if (pollCode.indexOf("w") === 0) return "mg/L";
  return null;
}
...
function parse(rawData) {
  try {
    ...
    if (crcStr) {
      ...
      var crcDisplay = crcUpper === computedHex
        ? crcUpper + " ✓"
        : crcUpper + " ✗ (预期 " + computedHex + ")";
      fields.push({ key: "_crc", label: "CRC16 校验", value: crcDisplay, group: "报文结构" });
    }
    ...
    for (var i = 0; i < headPairs.length; i++) {
      ...
      if (key === "QN") {
        fields.push({ key: "QN", label: "请求编码", value: val, group: "报文头" });
      } else if (key === "ST") {
        var stDisplay = ST_NAMES[val] ? val + " (" + ST_NAMES[val] + ")" : val;
        fields.push({ key: "ST", label: "系统编码", value: stDisplay, group: "报文头" });
      } else if (key === "CN") {
        cn = val;
        var cnDisplay = CN_NAMES[val] ? val + " (" + CN_NAMES[val] + ")" : val;
        fields.push({ key: "CN", label: "命令编码", value: cnDisplay, group: "报文头" });
      } else if (key === "PW") {
        fields.push({ key: "PW", label: "密码", value: val, group: "报文头" });
      } else if (key === "MN") {
        mn = val;
        fields.push({ key: "MN", label: "设备唯一标识", value: val, group: "报文头" });
      } else if (key === "Flag") {
        var flagVal = parseInt(val, 10) || 0;
        var d0 = flagVal & 0x01;
        var d1 = (flagVal >> 1) & 0x01;
        var flagDesc = val + " (D0=" + d0 + " " + (d0 ? "需应答" : "不应答") +
                       ", D1=" + d1 + " " + (d1 ? "有拆包" : "无拆包") + ")";
        fields.push({ key: "Flag", label: "标志位", value: flagDesc, group: "报文头" });
      } else if (key === "PNUM") {
...
function parseCpContent(cp, fields) {
  ...
  for (var j = 0; j < cpPairs.length; j++) {
    ...
    if (!cpPair) continue;
    var eqPos = cpPair.indexOf("=");
    if (eqPos < 0) continue;

    var cpKey = cpPair.substring(0, eqPos);
    var cpVal = cpPair.substring(eqPos + 1);
    ...
    if (KNOWN_CP_KEYS[cpKey]) {
      fields.push({ key: cpKey, label: KNOWN_CP_KEYS[cpKey], value: cpVal, group: "数据区" });
      continue;
    }
    // 通用 CP 键（非污染物因子）
